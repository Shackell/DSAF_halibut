---
title: "Temporal PCA Analysis of Shift Indicator Data"
author: "Shackell"
date: "`r Sys.Date()`"
output: 
  html_document:
...
Variable loadings show how each original variable contributes to the principal components. High positive or negative loadings indicate strong contributions.
...
kable(loadings_df %>% arrange(desc(abs(PC1))), digits = 3,
      caption = "Variable loadings on PC1 and PC2 (sorted by PC1 magnitude)")
```

```{r loadings-plot, fig.height=8}
# Get PC1 order for sorting
...
loadings_plot <- ggplot(loadings_long, aes(x = Variable, y = Loading, fill = Loading > 0)) +
  geom_col() +
...
combined_plot <- (p1 / p2) | loadings_plot
